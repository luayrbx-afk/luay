...
local function toggleAntiFling(state)
    ...
    if state then
        ...
        table.insert(antiFlingConnections, RunService.Stepped:Connect(function()
            ...
            for i = #unanchoredParts, 1, -1 do
                local p = unanchoredParts[i]
                if p and p.Parent then
                    ...
                    table.remove(unanchoredParts, i)
                end
            end
        end))
                    ...
                    elseif obj:IsA("Model") and not obj:FindFirstChildOfClass("Humanoid") then
                        for _, p in ipairs(obj:GetDescendants()) do
                            if p:IsA("BasePart") and not p.Anchored then
                                table.insert(unanchoredParts, p)
                            end
                        end
                    end
                end
...
-- == copy mibile save gboard limit poit1 == --
